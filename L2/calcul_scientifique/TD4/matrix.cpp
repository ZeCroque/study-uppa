...
void Matrix::factoCholesky(Matrix &B) const
{
	int i,j,k;
	double S;
	int n=(*this).getN();
	...
	for (i=0; i<n; i++)
		for(j=0; j<=i; j++)
		{
			...
			for(k=0; k<j; k++)
				S=S+B(i,k)*B(j,k);
			if (i==j)
				B(i,j)=sqrt((*this)(i,i)-S);
			else
				B(i,j)=((*this)(i,j)-S)/B(j,j);
		}					
}

void Matrix::solveCholesky(const std::vector<double> &b, std::vector<double> &x) const
{
	std::vector<double> y;
	Matrix B,BT;
	int n=(*this).getN();
	...
	if (b.size()==n)
	{
		...
		std::cout<<"Le vecteur entré n'a pas la bonne dimension."<<std::endl;		
