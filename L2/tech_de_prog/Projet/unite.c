...
void supprimerUnite(unite** uniTab, int iIndex, int* iSize)
{
	...
	if((*uniTab)[iIndex].altAssignee!=NULL)
	{
		for(i=0; i<*((*uniTab)[iIndex].altAssignee->iUniTabSize);i++)
		{
			if(((*uniTab)[iIndex].altAssignee)->uniTab[i]!=(&(*uniTab)[iIndex]))
			{
				for(;i<*((*uniTab)[iIndex].altAssignee->iUniTabSize-1);i++)
				{
					((*uniTab)[iIndex].altAssignee)->uniTab[i]=((*uniTab)[iIndex].altAssignee)->uniTab[i+1];
				}
				*((*uniTab)[iIndex].altAssignee->iUniTabSize)-=1;
			}
		}
	}

	free((*uniTab)[iIndex].strNom);
	free((*uniTab)[iIndex].strBase);
...
void afficherAlerteAssignee(unite uni)
{
	...
	{
		printf("\n\nCette unité n'est affectée à aucune alerte.\n\n");
	}
}
...
void gestionEtat(unite* uni)
{
	...
	do
	{
		system("clear");
		printf("*****GESTION ETAT*****\n\n");
		if(uni->iStatut==EN_ALERTE && uni->iDisponibilite!=EN_REPOS && uni->iDisponibilite!=EN_REAPPROVISIONNEMENT)
		{
			printf("L'unité est opérationnelle, veuillez l'affecter avant de modifier son état\n\n");
		}
		else if(uni->iStatut==EN_ROUTE)
		{
			printf("L'unité est elle arrivé à destination?\n[1]Oui\n[0]Non\n");
			scanf("%d%*c", &iUsrConfirm);
			if(iUsrConfirm)
			{
				uni->iStatut=SUR_OPERATION;
			}
		}
		else if(uni->iStatut==SUR_OPERATION)
		{
			printf("L'unité est en opération, il n'est pas possible de modifier son état pour le moment\n\n");
		}
		else if(uni->iDisponibilite==EN_REAPPROVISIONNEMENT)
		{
			printf("L'unité a-t-elle terminé de se réapprovisionner?\n[1]Oui\n[0]Non\n");
			scanf("%d%*c", &iUsrConfirm);
			if(iUsrConfirm)
			{
				if(uni->iCompteurOperation<3)
				{
						uni->iDisponibilite=ACTIVE;
				}
				else
				{
					uni->iCompteurOperation=0;
					uni->iDisponibilite=EN_REPOS;
				}
			}
		}
		else if(uni->iDisponibilite==EN_REPOS)
		{
			printf("L'unité a-t-elle terminé son repos?\n[1]Oui\n[0]Non\n");
			scanf("%d%*c", &iUsrConfirm);
			if(iUsrConfirm)
			{
				uni->iDisponibilite=ACTIVE;
		...
		if(!iUsrConfirm)
		{
			printf("L'état de l'unité n'a pas été modifié.\n");
		}
