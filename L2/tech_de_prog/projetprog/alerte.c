...
void assignerUnite(alerte* alt, unite* uni)
{
	...
	if (uni->altAssignee==NULL)
	{
		do
		{
			if (uni->iDisponibilite==EN_REAPPROVISIONNEMENT)
			{
			...
			if (uni->iDisponibilite==EN_REPOS)
			{
				printf("L'unité est en repos voulez vous la forcer à en sortir?\n[1]Oui\n[0]Non\n");
				scanf("%d%*c", &iUsrConfirm);
				if(iUsrConfirm)
				{
					uni->iDisponibilite=ACTIVE;
			...
			if(!iUsrConfirm)
			{
				printf("\nL'état de l'unité n'a pas été modifié.\n");
			}
		} while(iUsrConfirm<0 || iUsrConfirm>1);

		if(uni->iDisponibilite==ACTIVE)
		{
	...
	{
		printf("\nCette unité a déjà été déployée\n");
	}
}
...
void traiterAlerte(alerte** altTab, int iIndex, int* iSize)
{
	...
	if ((*altTab)[iIndex].iUniTabSize!=NULL && *((*altTab)[iIndex].iUniTabSize)!=0)
	{
		for(i=0; i<(*((*altTab)[iIndex].iUniTabSize));i++)
		{
			(*altTab)[iIndex].uniTab[i]->iDisponibilite=EN_REAPPROVISIONNEMENT;
			(*altTab)[iIndex].uniTab[i]->iStatut=EN_ALERTE;
		}
